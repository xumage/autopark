    ...
    if (date[0] && date[1]) {
      ...
      if (days > 0) {
        if (oneDay && deposit) {
          ...
          if (total > 0)
            $(".preview-total").html(`<strong>Итого: </strong>${total}руб`);
        }
      }
    } else if (date[0]) {
      let days = 1;
...
$(".submit-btn").on("click", function () {
  ...
  $(".modal-form-price-total").html(
    "Итоговая стоимость: " + priceTotal + " рублей"
  );
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data === "success") {
        ...
        $(".form-result").html(
          '<div class="send-success">Сообщение успешно отправлено!</div>'
        );
      } else {
        $('input[name="phone"], input[name="name"]').val("");
        $(".form-result").html(
          '<div class="send-error">Ошибка, повторите попытку через некоторое время!</div>'
        );
      }
      btn.text("Отправить");
